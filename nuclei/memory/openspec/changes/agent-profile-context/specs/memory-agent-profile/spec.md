...
MemoryConfig must accept an optional `agentProfile` field with the shape:
...
buildAmygdala must thread the agentProfile from MemoryConfig to the AmygdalaProcess constructor, following the same spread pattern as agentState.
...
AND the pattern matches the existing agentState threading at line 35
...
AND no conditional spread occurs for undefined agentProfile

### Requirement: Optional behavior without agentProfile

When agentProfile is absent from MemoryConfig, AmygdalaProcess initialization behavior is identical to the current implementation.
